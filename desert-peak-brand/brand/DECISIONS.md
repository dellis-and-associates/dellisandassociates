...
## Palette v2: Daniel's badge (2026-09-15)

**The palette is taken from the client's badge, not the badge from the palette.** Daniel's concept (`client/daniel-logo-concept.jpg`) is navy, sage and rust on cream. v1 of the refinement mapped those onto the Strata palette (navy → warm black, sage → agave teal, rust → hematite), which made the website's "logo tokens" pure aliases of the old colours and changed nothing on the page. `design-tokens.json` v2 moves the three ramps instead: `neutral` runs from warm paper (0–200, the badge's ground #FBF6F2) into Daniel's navy (400–950; 900 is #1E293B, the badge's ring and peak; 600 #44576C is the far peak); `brand` is navy at hue 260 (600 #243858, a step lighter than ink so a button is not the text colour; the concept's rust INSURANCE line is not in the web lockup and was dropped from the palette on 2026-09-15 after a first pass used it as the action colour); `positive` is sage at hue 148 through 500 (#698E6E, the saguaro, 3.4:1, graphics only) and cools toward fir from 600 down (700 #2C5041 for state text) because pure sage at text darkness fails the colour-vision gate against notice (protanopia ΔE 0.069) and critical (deuteranopia 0.072); the fir hue passes both with ΔE ≥ 0.085. `accent` (ochre), `notice` and `critical` are unchanged. Every semantic alias is unchanged, so components did not move.

**The badge's saguaro is `positive.500`, a primitive.** The web lockups (`logo/daniel-refined/web`) and the site's `--dp-logo-cactus` point at the mid sage, which is a 3:1 graphic, not the 4.5:1 semantic `positive`. It is the one place the site references a primitive step, recorded here so the guide's rule ("components never reference ramps") stays honest.

**The Strata logo files were recoloured, not redrawn.** `brand/logos/*.svg` had their four hexes substituted (hematite → navy brand 600, warm black → navy 900, cream → paper 50/0) so the contrast audit's "every logo hex is a token" check holds, and `MANIFEST.sha256` was re-locked. They are no longer the site's logo; the Stage 5 collateral built on them (`social/`, `email/`, `decks/`, `web/`, `print/`, `showcase/`) still carries v1 hexes and fails `verify:token-lock` until it is regenerated on the badge or retired. `BRAND-GUIDE.md` still describes v1 colour names (hematite, agave); the token file wins.
