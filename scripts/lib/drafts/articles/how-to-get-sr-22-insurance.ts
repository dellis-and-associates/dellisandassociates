...
export const draft: ArticleDraft = {
  ...
  sections: [
    ...
    },
    {
      ...
      paragraphs: [
        "Read the order. It will name the filing period, which is set by each state and by the offence, and it will say whether the period runs from the conviction, from the licence suspension, or from the reinstatement; it can also state a minimum liability limit that the certified policy must carry. The period and limits differ by state, and the order itself, or the state motor vehicle agency that issued it, is the authority for both. The filing period is continuous: any lapse in the certified policy during it is reported to the state, the licence is suspended again, and in most cases the period starts over from the date coverage resumes.",
