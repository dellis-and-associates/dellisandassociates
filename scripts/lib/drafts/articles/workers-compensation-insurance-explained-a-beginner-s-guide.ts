...
export const draft: ArticleDraft = {
  ...
  sections: [
    ...
    },
    {
      ...
      paragraphs: [
        "Each of the four states sets its own threshold for when an employer must have coverage, its own rules on whether sole proprietors, partners, LLC members and corporate officers are counted as employees or may elect out, and its own penalties for going without, published by the Arizona Industrial Commission, the Nevada Department of Business and Industry, the Utah Labor Commission and the Idaho Industrial Commission. The pattern is that the threshold is low, often the first employee, and the exemptions for owners are elections that have to be filed rather than assumptions.",
