...
export default async function Glossary() {
  ...
  return (
    <div className="mx-auto grid max-w-measure-shell gap-10 px-gutter py-10">
      ...
      <nav aria-label="Jump to letter" className="flex flex-wrap gap-1 font-text text-copy">{letters.map((l) => <a key={l} href={`#letter-${l}`} className="ui-link inline-flex min-h-11 min-w-11 items-center justify-center rounded-control border border-border text-ink">{l}</a>)}</nav>
      {letters.map((l) => (
        <section key={l} aria-labelledby={`letter-${l}`} className="grid gap-3">
          <h2 id={`letter-${l}`} className="font-text text-title-sm">{l}</h2>
          <ul className="grid gap-2 font-text text-copy sm:grid-cols-2 lg:grid-cols-3">{byLetter.get(l)!.map((t) => <li key={t.id}><Link href={glossaryPath(t.slug)} className="ui-link text-ink">{t.term}</Link>{t.reviewStatus !== "reviewed" ? <span className="ml-2 text-ink-muted">(draft)</span> : null}</li>)}</ul>
        </section>
      ))}
    </div>
  );
