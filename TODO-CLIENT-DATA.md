...
| 12 | Arizona and Idaho Department of Insurance links (`difi.az.gov` answers 403 to the build machine, `doi.idaho.gov` did not answer); confirm the URLs so the TODO tokens on `States.doi` can be filled | 2026-09-12 | State hubs, `/legal/licensing/arizona/`, `/legal/licensing/idaho/` |
| 13 | Decision on two routes not in the IA: `/forms/new-client-intake/` and `/forms/medication-intake/` (legacy parity rows 7–8; +2 routes), and the partner-portal path for the legacy agent pages | 2026-09-12 | `REDIRECTS-UNRESOLVED.md` (6 legacy URLs) |
