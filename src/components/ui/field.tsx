...
const CONTROL = "block w-full min-h-11 rounded-control border bg-surface-raised px-3 font-sans text-body text-ink placeholder:text-ink-muted";
...
function Wrap({ id, label, help, error, required, optionalLabel = true, children }: Common & { children: ReactNode }) {
  return (
    <div className="grid gap-1">
      <label htmlFor={id} className="font-sans text-small font-semibold text-ink">
        {label}
        {!required && optionalLabel ? <span className="ml-1 font-normal text-ink-muted">(optional)</span> : null}
      </label>
      {children}
      {help && !error ? <p id={`${id}-help`} className="font-sans text-small text-ink-muted">{help}</p> : null}
      {error ? (
        <p id={`${id}-error`} className="font-sans text-caption font-semibold text-critical">
          {error}
        </p>
      ) : null}
    </div>
...
export function ChoiceGroup({ id, legend, name, type, options, help, error, required, defaultValues = [] }: { id: string; legend: string; name: string; type: "checkbox" | "radio"; options: { label: string; value: string; help?: string }[]; help?: string; error?: string; required?: boolean; defaultValues?: string[] }) {
  return (
    <fieldset id={id} className="grid gap-2" aria-describedby={describedBy(id, help, error)} aria-invalid={error ? true : undefined}>
      <legend className="mb-1 font-sans text-small font-semibold text-ink">{legend}{required ? null : <span className="ml-1 font-normal text-ink-muted">(optional)</span>}</legend>
      {options.map((o) => (
        <label key={o.value} className="flex min-h-6 items-start gap-3 font-sans text-small text-ink">
      ...
      {help && !error ? <p id={`${id}-help`} className="font-sans text-small text-ink-muted">{help}</p> : null}
      {error ? <p id={`${id}-error`} className="font-sans text-caption font-semibold text-critical">{error}</p> : null}
    </fieldset>
  );
}
...
export function ErrorSummary({ errors, id = "error-summary" }: { errors: { field: string; message: string }[]; id?: string }) {
  ...
  return (
    <div id={id} role="alert" tabIndex={-1} className="rounded-surface border border-critical-border border-l-4 bg-critical-surface p-4 text-critical-ink">
      <p className="font-sans text-small font-semibold">There {errors.length === 1 ? "is 1 thing" : `are ${errors.length} things`} to fix before this can be sent.</p>
      <ul className="mt-2 grid gap-1 font-sans text-small">
        {errors.map((e) => (
