...
export function QuoteSummaryForm({ data, productNames }: { data: QuoteData; productNames: string[] }) {
  ...
  const row = (label: string, value?: string | null) => (
    <div className="grid grid-cols-[minmax(0,10rem)_1fr] gap-3 py-2 font-sans text-small"><dt className="text-ink-muted">{label}</dt><dd className="tabular">{value || <span className="text-ink-muted">not given</span>}</dd></div>
  );
  const section = (title: string, step: number, children: React.ReactNode) => (
    <section aria-labelledby={`s-${step}`} className="rounded-surface border border-border bg-surface-raised p-5">
      <div className="flex items-center justify-between gap-4"><h2 id={`s-${step}`} className="font-sans text-title-sm">{title}</h2><Link href={`/quote/${step}/`} className="ui-link font-sans text-small underline">Edit {title.toLowerCase()}</Link></div>
      <dl className="mt-3 divide-y divide-border">{children}</dl>
    </section>
  );
  return (
    <div className="mx-auto grid max-w-measure-page gap-8 px-4 py-10 md:px-8" data-quote-summary>
      <div className="grid max-w-measure-body gap-6">
        ...
        <form action={formAction} method="post" className="grid gap-6" noValidate aria-busy={pending || undefined}>
          ...
          <fieldset className="grid gap-2" aria-describedby={errors.find((e) => e.field === "consent") ? "consent-error" : undefined}>
            ...
            <label htmlFor="consent" className="flex items-start gap-3 font-sans text-small">
            ...
            {errors.find((e) => e.field === "consent") ? <p id="consent-error" className="font-sans text-caption font-semibold text-critical">{errors.find((e) => e.field === "consent")!.message}</p> : null}
          </fieldset>
          <div className="cf-turnstile" data-sitekey={publicEnv.TURNSTILE_SITE_KEY} data-size="flexible" />
          <script src="https://challenges.cloudflare.com/turnstile/v0/api.js" async defer />
          <div className="flex flex-wrap items-center gap-4">
            <button className="inline-flex min-h-11 items-center rounded-control bg-brand px-5 font-sans text-small font-semibold text-brand-ink hover:bg-brand-hover" disabled={pending}>Request my comparison</button>
            <Link href="/quote/3/" className="ui-link font-sans text-small underline">Back</Link>
          </div>
        </form>
      </div>
    </div>
