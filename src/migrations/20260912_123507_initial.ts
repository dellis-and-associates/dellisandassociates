...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  // Payload creates the schema only in push mode; a migration-only database (Supabase, CI) needs it created here.
  await db.execute(sql`CREATE SCHEMA IF NOT EXISTS "payload_cms";`)
  await db.execute(sql`
   CREATE TYPE "payload_cms"."enum_users_roles" AS ENUM('admin', 'editor', 'agent', 'partner');
  CREATE TABLE "payload_cms"."users_roles" (
  	"order" integer NOT NULL,
  	"parent_id" integer NOT NULL,
  	"value" "payload_cms"."enum_users_roles",
  ...
  ALTER TABLE "payload_cms"."users_roles" ADD CONSTRAINT "users_roles_parent_fk" FOREIGN KEY ("parent_id") REFERENCES "payload_cms"."users"("id") ON DELETE cascade ON UPDATE no action;
  ...
  CREATE INDEX "users_roles_order_idx" ON "payload_cms"."users_roles" USING btree ("order");
  CREATE INDEX "users_roles_parent_idx" ON "payload_cms"."users_roles" USING btree ("parent_id");
  ...
  CREATE INDEX "media_sizes_card_sizes_card_filename_idx" ON "payload_cms"."media" USING btree ("sizes_card_filename");
  CREATE INDEX "media_sizes_content_sizes_content_filename_idx" ON "payload_cms"."media" USING btree ("sizes_content_filename");
  CREATE INDEX "media_sizes_hero_sizes_hero_filename_idx" ON "payload_cms"."media" USING btree ("sizes_hero_filename");
...
export async function down({ db, payload, req }: MigrateDownArgs): Promise<void> {
  await db.execute(sql`
   DROP TABLE "payload_cms"."users_roles" CASCADE;
  ...
  DROP TYPE "payload_cms"."enum_users_roles";`)
}
