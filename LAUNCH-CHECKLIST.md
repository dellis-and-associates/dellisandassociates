...
| C6a | [ ] | Security headers are live on the deployed domain | engineer | `curl -sI https://www.desertpeakinsurance.com/` shows `Content-Security-Policy`, `Referrer-Policy`, `X-Content-Type-Options`, `X-Frame-Options`, `Permissions-Policy` and `Cross-Origin-Opener-Policy`. They ship with the app (`next.config.ts`), so they arrive with the next deploy; securityheaders.com re-scans on demand |
| C6b | [ ] | Decide on HSTS `includeSubDomains` and `preload` | client, with the engineer | Vercel already sends `max-age=63072000`. `includeSubDomains` forces HTTPS on every current **and future** subdomain (mail, tracking, a vendor's portal — anything served over HTTP stops working). `preload` submits the domain to a browser-shipped list and is slow and awkward to undo. Both are one line in `next.config.ts` once the answer is yes; neither is a decision to take by default |
